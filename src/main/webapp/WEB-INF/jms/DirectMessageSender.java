...
public class DirectMessageSender {
    ...
    public DirectMessageSender() {
        try {
            ...
            KeyValueTable kv = new KeyValueTable(1, "anton");
            ObjectMessage outObj = session.createObjectMessage(kv);
            System.out.println("Object message: " + kv.key);
    ...
    public DirectMessageSender(String topic, String filter) {
        try {
            ...
            producer = session.createProducer(dest);
            KeyValueTable kv = new KeyValueTable(1, "PUBG_LOVERS");
            TextMessage outObj = session.createTextMessage();
            outObj.setText(kv.value);
            outObj.setStringProperty("symbol", filter);
            System.out.println("Object message: " + kv.value);
            producer.send(outObj);
            connection.start();
        } catch (NamingException | JMSException e) {
            e.printStackTrace();
